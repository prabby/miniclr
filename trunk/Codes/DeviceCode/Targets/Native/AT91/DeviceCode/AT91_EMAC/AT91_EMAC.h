...
struct AT91_EMAC {
    ...
    static const    UINT32 EMAC_LB         = (0x1 <<  0); // (EMAC) Loopback. Optional. When set, loopback signal is at high level.
    static const    UINT32 EMAC_LLB        = (0x1 <<  1); // (EMAC) Loopback local. 
    static const    UINT32 EMAC_RE         = (0x1 <<  2); // (EMAC) Receive enable. 
    static const    UINT32 EMAC_TE         = (0x1 <<  3); // (EMAC) Transmit enable. 
    static const    UINT32 EMAC_MPE        = (0x1 <<  4); // (EMAC) Management port enable. 
    static const    UINT32 EMAC_CLRSTAT    = (0x1 <<  5); // (EMAC) Clear statistics registers. 
    static const    UINT32 EMAC_INCSTAT    = (0x1 <<  6); // (EMAC) Increment statistics registers. 
    static const    UINT32 EMAC_WESTAT     = (0x1 <<  7); // (EMAC) Write enable for statistics registers. 
    static const    UINT32 EMAC_BP         = (0x1 <<  8); // (EMAC) Back pressure. 
    static const    UINT32 EMAC_TSTART     = (0x1 <<  9); // (EMAC) Start Transmission. 
    static const    UINT32 EMAC_THALT      = (0x1 << 10); // (EMAC) Transmission Halt. 
    static const    UINT32 EMAC_TPFR       = (0x1 << 11); // (EMAC) Transmit pause frame 
    static const    UINT32 EMAC_TZQ        = (0x1 << 12); // (EMAC) Transmit zero quantum pause frame

    /****/ volatile UINT32 EMAC_NCFGR;    // Network Configuration Register
    static const    UINT32 EMAC_SPD           = (0x1 <<  0); // (EMAC) Speed. 
    static const    UINT32 EMAC_FD            = (0x1 <<  1); // (EMAC) Full duplex. 
    static const    UINT32 EMAC_JFRAME        = (0x1 <<  3); // (EMAC) Jumbo Frames. 
    static const    UINT32 EMAC_CAF           = (0x1 <<  4); // (EMAC) Copy all frames. 
    static const    UINT32 EMAC_NBC           = (0x1 <<  5); // (EMAC) No broadcast. 
    static const    UINT32 EMAC_MTI           = (0x1 <<  6); // (EMAC) Multicast hash event enable
    static const    UINT32 EMAC_UNI           = (0x1 <<  7); // (EMAC) Unicast hash enable. 
    static const    UINT32 EMAC_BIG           = (0x1 <<  8); // (EMAC) Receive 1522 bytes. 
    static const    UINT32 EMAC_EAE           = (0x1 <<  9); // (EMAC) External address match enable. 
    static const    UINT32 EMAC_CLK           = (0x3 << 10); // (EMAC) 
    static const    UINT32 EMAC_CLK_HCLK_8    = (0x0 << 10); // (EMAC) HCLK divided by 8
    static const    UINT32 EMAC_CLK_HCLK_16   = (0x1 << 10); // (EMAC) HCLK divided by 16
    static const    UINT32 EMAC_CLK_HCLK_32   = (0x2 << 10); // (EMAC) HCLK divided by 32
    static const    UINT32 EMAC_CLK_HCLK_64   = (0x3 << 10); // (EMAC) HCLK divided by 64
    static const    UINT32 EMAC_RTY           = (0x1 << 12); // (EMAC) 
    static const    UINT32 EMAC_PAE           = (0x1 << 13); // (EMAC) 
    static const    UINT32 EMAC_RBOF          = (0x3 << 14); // (EMAC) 
    static const    UINT32 EMAC_RBOF_OFFSET_0 = (0x0 << 14); // (EMAC) no offset from start of receive buffer
    static const    UINT32 EMAC_RBOF_OFFSET_1 = (0x1 << 14); // (EMAC) one byte offset from start of receive buffer
    static const    UINT32 EMAC_RBOF_OFFSET_2 = (0x2 << 14); // (EMAC) two bytes offset from start of receive buffer
    static const    UINT32 EMAC_RBOF_OFFSET_3 = (0x3 << 14); // (EMAC) three bytes offset from start of receive buffer
    static const    UINT32 EMAC_RLCE          = (0x1 << 16); // (EMAC) Receive Length field Checking Enable
    static const    UINT32 EMAC_DRFCS         = (0x1 << 17); // (EMAC) Discard Receive FCS
    static const    UINT32 EMAC_EFRHD         = (0x1 << 18); // (EMAC) 
    static const    UINT32 EMAC_IRXFCS        = (0x1 << 19); // (EMAC) Ignore RX FCS

    /****/ volatile UINT32 EMAC_NSR;      // Network Status Register
    static const    UINT32 EMAC_LINKR      = (0x1 <<  0); // (EMAC) 
    static const    UINT32 EMAC_MDIO       = (0x1 <<  1); // (EMAC) 
    static const    UINT32 EMAC_IDLE       = (0x1 <<  2); // (EMAC) 

    /****/ volatile UINT32 Reserved0[2];  

    /****/ volatile UINT32 EMAC_TSR;      // Transmit Status Register
    static const    UINT32 EMAC_UBR        = (0x1 <<  0); // (EMAC) 
    static const    UINT32 EMAC_COL        = (0x1 <<  1); // (EMAC) 
    static const    UINT32 EMAC_RLES       = (0x1 <<  2); // (EMAC) 
    static const    UINT32 EMAC_TGO        = (0x1 <<  3); // (EMAC) Transmit Go
    static const    UINT32 EMAC_BEX        = (0x1 <<  4); // (EMAC) Buffers exhausted mid frame
    static const    UINT32 EMAC_COMP       = (0x1 <<  5); // (EMAC) 
    static const    UINT32 EMAC_UND        = (0x1 <<  6); // (EMAC) 
    ...
    static const    UINT32 EMAC_BNA        = (0x1 <<  0); // (EMAC) 
    static const    UINT32 EMAC_REC        = (0x1 <<  1); // (EMAC) 
    static const    UINT32 EMAC_OVR        = (0x1 <<  2); // (EMAC) 

    /****/ volatile UINT32 EMAC_ISR;      // Interrupt Status Register
    static const    UINT32 EMAC_MFD        = (0x1 <<  0); // (EMAC) 
    static const    UINT32 EMAC_RCOMP      = (0x1 <<  1); // (EMAC) 
    static const    UINT32 EMAC_RXUBR      = (0x1 <<  2); // (EMAC) 
    static const    UINT32 EMAC_TXUBR      = (0x1 <<  3); // (EMAC) 
    static const    UINT32 EMAC_TUNDR      = (0x1 <<  4); // (EMAC) 
    static const    UINT32 EMAC_RLEX       = (0x1 <<  5); // (EMAC) 
    static const    UINT32 EMAC_TXERR      = (0x1 <<  6); // (EMAC) 
    static const    UINT32 EMAC_TCOMP      = (0x1 <<  7); // (EMAC) 
    static const    UINT32 EMAC_LINK       = (0x1 <<  9); // (EMAC) 
    static const    UINT32 EMAC_ROVR       = (0x1 << 10); // (EMAC) 
    static const    UINT32 EMAC_HRESP      = (0x1 << 11); // (EMAC) 
    static const    UINT32 EMAC_PFRE       = (0x1 << 12); // (EMAC) 
    static const    UINT32 EMAC_PTZ        = (0x1 << 13); // (EMAC) 
    ...
    static const    UINT32 EMAC_DATA       = (0xFFFF <<  0); // (EMAC) 
    static const    UINT32 EMAC_CODE       = (0x03 << 16);   // (EMAC) 
    static const    UINT32 EMAC_REGA       = (0x1F << 18);   // (EMAC) 
    static const    UINT32 EMAC_PHYA       = (0x1F << 23);   // (EMAC) 
    static const    UINT32 EMAC_RW         = (0x03 << 28);   // (EMAC) 
    static const    UINT32 EMAC_SOF        = (0x03 << 30);   // (EMAC) 
    ...
    static const    UINT32 EMAC_RMII       = (0x1 <<  0);       // (EMAC) Reduce MII

    static const    UINT32 EMAC_CLKEN      = (0x1 <<  1);       // (EMAC) Clock Enable

    /****/ volatile UINT32 EMAC_WOL;      // Wake On LAN Register
    static const    UINT32 EMAC_IP         = (0xFFFF <<  0);    // (EMAC) ARP request IP address

    static const    UINT32 EMAC_MAG        = (0x1 << 16);       // (EMAC) Magic packet event enable

    static const    UINT32 EMAC_ARP        = (0x1 << 17);       // (EMAC) ARP request event enable

    static const    UINT32 EMAC_SA1        = (0x1 << 18);       // (EMAC) Specific address register 1 event enable

    /****/ volatile UINT32 Reserved1[13];  

    /****/ volatile UINT32 EMAC_REV;      // Revision Register
    static const    UINT32 EMAC_REVREF     = (0xFFFF <<  0);    // (EMAC) 
    static const    UINT32 EMAC_PARTREF    = (0xFFFF << 16);    // (EMAC) 
