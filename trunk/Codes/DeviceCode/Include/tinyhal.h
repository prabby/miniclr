...
public:
    UINT16 m_maxValues;
    UINT16 m_numValues;
    float  m_averagedValue;
    float  m_addFraction;
    float  m_scaleFraction;
...
public:
    ...
    INT32 Get() const { return (INT32)m_averagedValue; }
...
#define SYSTEM_EVENT_FLAG_UNUSED_0x00000800         0x00000800
...
public:
    ...
    void Unlink()
    {
        ...
        if(prev && next)
        {
            prev->m_nextNode = next; m_nextNode = NULL;
            next->m_prevNode = prev; m_prevNode = NULL;
        }
    }
};
...
public:
    ...
    T* Pop()
    {
        ...
        if(m_reader == m_writer && !m_full) return NULL;
...
#define HAL_DECLARE_CUSTOM_HEAP(allocFtn,freeFtn)                      \
    extern "C" {                                                   \
