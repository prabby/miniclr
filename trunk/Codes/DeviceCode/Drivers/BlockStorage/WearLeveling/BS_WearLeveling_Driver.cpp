...
BOOL BS_WearLeveling_Driver::InitializeDevice(void *context)
{
    ...
    UINT32          Blocks;
    ByteAddress     BlockAddress    = pDevInfo->Regions[0].Start;
    ...
    {       
        ...
        while(Blocks--)
        {
            ...
            {
                UINT16 badIndex = BlockAddress / config->BytesPerBlock;
...
{
    ByteAddress freeBlock      = config->BlockConfig->BlockDeviceInformation->Size - config->BytesPerBlock;
    UINT16      BadBlockIndex  = BadBlockAddress / config->BytesPerBlock;
    ...
    pBadMap->VirtualBlockIndex  = BadBlockAddress / config->BytesPerBlock;
    pBadMap->PhysicalBlockIndex = freeBlock       / config->BytesPerBlock;
    ...
    meta.wOwnerBlock = BadBlockAddress / config->BytesPerBlock;
...
BOOL BS_WearLeveling_Driver::CompactBlocks(BS_WearLeveling_Config* config, ByteAddress virtSectAddress)
{
    ...
    ByteAddress     tmpAddress      = (NumBlocks-1) * config->BytesPerBlock;
...
//debug_printf("BlockCompaction\r\n");    
    ...
    while(NumBlocks--)
    {
          ...
          (meta.IsBlockFree() || (meta.IsBlockTrash() && !meta.IsValidBlockMapOffset())))
        {
            ...
            {
                CopyBlockAddress1 = NumBlocks * config->BytesPerBlock;
            }
            else
            {
                CopyBlockAddress2 = NumBlocks * config->BytesPerBlock;
                break;
            }
        }
        ...
        for(UINT32 SectorIndex=0; SectorIndex < SectorsPerBlock; SectorIndex++)
        {
            ...
            else if(SectorIndex == 0)
            {
                ...
                else if(meta.IsValidBlockMapOffset())
                {
                    ...
                    UINT32 MappedBlock = meta.GetBlockMapOffset() * config->BytesPerBlock;
                    ...
                    while(TRUE)
                    {
                        ...
                        if(!meta.IsBlockTrash() && meta.IsValidOwnerBlock())
                        {
                            ...
                            NextBlock   = meta.wOwnerBlock * config->BytesPerBlock;
    ...
    while(TRUE)
    {
        ...
        while(TRUE)
        {
            for(int i=maxValue-1; i>0; i--)
            {
                ...
                {
                    phySectAddr = (i | curBlockIndex) * config->BytesPerBlock;
...
{
    UINT16 virtualBlockIndex        = (UINT16)(virtAddress / config->BytesPerBlock);
    ...
    if(config->Device->GetSectorMetadata( config->BlockConfig, virtBlockAddress, (SectorMetadata*)&meta ) && !meta.IsBadBlock())
    {
        ...
        {
            phyBlockAddress = meta.GetBlockMapOffset() * config->BytesPerBlock;
    ...
    else
    {
        ...
        while(pBadList != NULL)
        {
            ...
            {
                phyBlockAddress = pBadList->PhysicalBlockIndex * config->BytesPerBlock;
                return TRUE;
            }
...
SetBlockColor( currentBlockAddr, COLOR_RED, config->BytesPerBlock );
    ...
    if(virtBlockAddress != phyNewBlockAddress)
    {
        ...
        UINT32 phyNewBlockIndex = phyNewBlockAddress / config->BytesPerBlock;
    ...
    {
        meta.SetBlockMapped();
        meta.wOwnerBlock = virtBlockAddress / config->BytesPerBlock;
    }            
...
BOOL BS_WearLeveling_Driver::GetPhysicalAddress(BS_WearLeveling_Config* config, ByteAddress virtAddress, ByteAddress &phyAddress, BOOL &fDirectSectorMap)
{
    ...
    ASSERT(phyAddress < ((config->BlockIndexMask << 1) * config->BytesPerBlock));
...
if(g_pLcdBuffer)
{
    ...
    { 
        ...
        for(int i=0; i<(config->BlockConfig->BlockDeviceInformation->Regions[0].NumBlocks); i++)
        {
            ...
            config->Device->GetSectorMetadata(config->BlockConfig, i*config->BytesPerBlock, (SectorMetadata*)&virtMeta);
