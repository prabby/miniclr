...
HRESULT FAT_FileHandle::Close()
{
    ...
    if (dirEntry && m_readWriteState != ReadWriteState__NONE)
    {
        ...
        if(dirEntry->DIR_LstAccDate != date)
        {            
            m_file.MarkDirectoryEntryForWrite();
            markedForWrite = TRUE;
            
            dirEntry->DIR_LstAccDate = date;
        }
...
HRESULT FAT_FileHandle::Read( BYTE* buffer, int size, int* bytesRead )
{
    ...
    if(m_position > dirEntry->DIR_FileSize)
    {
        m_position = dirEntry->DIR_FileSize;
    }

    limit = dirEntry->DIR_FileSize - m_position;
...
HRESULT FAT_FileHandle::Write( BYTE* buffer, int size, int* bytesWritten )
{
    ...
    //if file is resized
    if(dirEntry->DIR_FileSize < m_position)
    {
        m_file.MarkDirectoryEntryForWrite();
        dirEntry->DIR_FileSize = m_position;
    }
...
HRESULT FAT_FileHandle::Seek( INT64 offset, UINT32 origin, INT64* position )
{
    ...
    case SEEKORIGIN_CURRENT:
        ...
        break;
    case SEEKORIGIN_END:
        newPosition = dirEntry->DIR_FileSize + offset;
        break;
    default:
        TINYCLR_SET_AND_LEAVE(CLR_E_INVALID_PARAMETER);
    }
    ...
    if(newPosition > dirEntry->DIR_FileSize)
    {
...
HRESULT FAT_FileHandle::SetLength( INT64 length )
{
    ...
    if((length32 == dirEntry->DIR_FileSize) ||
    ...
    if(length32 <= dirEntry->DIR_FileSize)
    {
        ...
        UINT32 oldClusCount = (dirEntry->DIR_FileSize + clusterSize - 1) / clusterSize;
        ...
        dirEntry->DIR_FileSize = length32;
    ...
    else //growing
    {
        ...
        if(length32 - dirEntry->DIR_FileSize > m_logicDisk->GetDiskFreeSize())
        {
            TINYCLR_SET_AND_LEAVE(CLR_E_FILE_IO);
        }
