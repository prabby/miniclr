    ...
    else if(countOfClusters < 65525)
    {
        ...
        *isFAT16 = TRUE;
        ...
        (
            ...
            (Get_BPB_TotSec16() == 0 || BPB_TotSec32 == 0)
        );
    }
    else
    {
...
void FAT_FSINFO::Initialize( UINT32 freeCount, UINT32 nxtFree )
{
    ...
    FSI_Free_Count = freeCount;
    FSI_Nxt_free   = nxtFree;
...
void FAT_LONG_Directory::SetName( LPCWSTR name, UINT32 nameLen )
{
    ...
    for(j = 0; j < LDIR_Name2__size / sizeof(UINT16); j++)
    {
        LDIR_Name2[j] = *name;
        name++;
    }

    for(j = 0; j < LDIR_Name3__size / sizeof(UINT16); j++)
    {
        LDIR_Name3[j] = *name;
        name++;
    }
...
BOOL FAT_LONG_Directory::IsName( LPCWSTR name, UINT32 nameLen )
{
    ...
    for(j = 0; j < LDIR_Name2__size / sizeof(UINT16); j++)
    {
        if(LDIR_Name2[j] == 0 && nameLen == 0)
        {
            return TRUE;
        }
        else if(nameLen == 0 || MF_towupper( LDIR_Name2[j] ) != MF_towupper( *name ))
        {
            return FALSE;
        }
    ...
    for(j = 0; j < LDIR_Name3__size / sizeof(UINT16); j++)
    {
        if(LDIR_Name3[j] == 0 && nameLen == 0)
        {
            return TRUE;
        }
        else if(nameLen == 0 || MF_towupper( LDIR_Name3[j] ) != MF_towupper( *name ))
        {
            return FALSE;
        }
