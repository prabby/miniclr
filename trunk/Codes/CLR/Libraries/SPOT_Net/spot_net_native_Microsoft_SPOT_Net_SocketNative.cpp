...
HRESULT Library_spot_net_native_Microsoft_SPOT_Net_SocketNative::socket___STATIC__I4__I4__I4__I4( CLR_RT_StackFrame& stack )
{
    ...
    int nonBlocking = 1;
    int sock        = SOCK_socket( family, type, protocol );
...
HRESULT Library_spot_net_native_Microsoft_SPOT_Net_SocketNative::poll___STATIC__BOOLEAN__OBJECT__I4__I4( CLR_RT_StackFrame& stack )
{
    ...
    int timeout_us = stack.Arg2().NumericByRef().s4;
...
HRESULT Library_spot_net_native_Microsoft_SPOT_Net_SocketNative::SendRecvHelper( CLR_RT_StackFrame& stack, bool fSend, bool fAddress )
{
    ...
    while(count > 0)
    {
        ...
        // UDP returns immediately after receiving bytes.
        if(fAddress && !fSend && (totReadWrite > 0))
        {
            break;
        }
...
void Library_spot_net_native_Microsoft_SPOT_Net_SocketNative::ThrowError( CLR_RT_StackFrame& stack, int errorCode )
{        
    NATIVE_PROFILE_CLR_NETWORK();
...
HRESULT Library_spot_net_native_Microsoft_SPOT_Net_SocketNative::ThrowOnError( CLR_RT_StackFrame& stack, int res )
{        
    NATIVE_PROFILE_CLR_NETWORK();
    TINYCLR_HEADER();
