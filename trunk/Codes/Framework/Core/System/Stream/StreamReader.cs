...
namespace System.IO
{
    public class StreamReader : TextReader
    {
        ...
        public override int Read(char[] buffer, int index, int count)
        {
            ...
            Decoder dec = this.CurrentEncoding.GetDecoder();

            while(true)
            {
                dec.Convert( m_buffer, m_curBufPos, m_curBufLen - m_curBufPos, buffer, 0, count, false, out byteUsed, out charUsed, out completed);

                if (charUsed == count || (FillBufferAndReset( count ) == 0))
                {
                    break;
                }
            }
        ...
        private int ReadByte()
        {
            ...
            if (m_curBufPos == m_curBufLen)
                if (FillBufferAndReset( 1 ) == 0)
                    return -1;
            return m_buffer[m_curBufPos++];
        }

        internal int ReadBytes(byte[] buffer, int index, int count)
        {
            if (buffer == null)
                throw new ArgumentNullException();
            if (index < 0)
            ...
            // Make sure we have enough to read. If not read remaining then go
            // straight to the stream until complete
            if (m_curBufPos + count >= m_curBufLen)
            {
                // Copy the remainder of the buffer
                int totalRead = 0;
                int remainder;
                if ((remainder = m_curBufLen - m_curBufPos) > 0)
                    Array.Copy(m_buffer, m_curBufPos, buffer, index, remainder);
                count -= remainder;
                m_curBufPos += remainder;
                index += remainder;
                totalRead += remainder;

                // Skip the buffering and go straight to the stream for the remainder
                int noRead = 0;
                while (count > 0)
                {
                    noRead = m_stream.Read(buffer, index, count);
            ...
            // Else copy the buffer and advance pointers
            Array.Copy(m_buffer, m_curBufPos, buffer, index, count);
            m_curBufPos += count;
