...
namespace Dpws.Device.Services
{
    ...
    public class DpwsWseEventSource
    {
        ...
        private DpwsWseEventSinks m_eventSinks = new DpwsWseEventSinks();
    ...
    public class DpwsWseEventSources
    {
        ...
        private ArrayList m_sourceList = new ArrayList();
    ...
    internal class DpwsWseEventSink
    {
        ...
        private WsWsaEndpointRef m_endTo = null;
        private WsWsaEndpointRef m_notifyTo = null;
    ...
    {
        private object m_threadLock = new object();
        private ArrayList m_sinkList = new ArrayList();
        ...
        public int Add(DpwsWseEventSink value)
        {
            lock (m_threadLock)
            {
                ...
                int count = m_sinkList.Count;
                String valueUri = value.NotifyTo.Address.AbsoluteUri;
                for (int i = 0; i < count; i++)
                {
                    if (((DpwsWseEventSink)m_sinkList[i]).NotifyTo.Address.AbsoluteUri == valueUri)
                    {
