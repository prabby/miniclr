...
namespace Ws.Services.Mtom
{
    ...
    {
        private ArrayList m_partsList = new ArrayList();
    ...
    internal class WsMtom
    {
        ...
        private int m_growSize = 0xFFFF;
        private int m_capacity = 0xFFFF;
        private int m_maxCapacity = 0xFFFFF;
        ...
        public byte[] ParseMessage(string boundary)
        {
            ...
            while (m_curPos < m_length)
            {
                ...
                while (readBuffer.Length != 0)
                {
                    ...
                    string[] headerEntry = readBuffer.Split(':');
                    if (headerEntry.Length == 2)
                    {
                        switch (headerEntry[0])
                        {
                            case "Content-Type":
                                bodyPart.ContentType = headerEntry[1].TrimStart(' ');
                                break;
                            case "Content-Transfer-Encoding":
                                bodyPart.ContentTransferEncoding = headerEntry[1].TrimStart(' ');
                                break;
                            case "Content-ID":
                                bodyPart.ContentID = headerEntry[1].TrimStart(' ');
                                break;
                            default:
                ...
                if (bodyPart.ContentTransferEncoding != "binary")
        ...
        private string ReadLine()
        {
            ...
            while (true)
            {
                // Check on the read buffer. If we get to the end before
                // \r\n is found don't return null.
                if (m_curPos >= lastBufPos)
                    break;

                // if \r found on previous loop check for \n
                // if found return m_curPos.
                // if \r is found but no trailing \n is found keep looking.
                if (crFound)
                {
                    if (m_buffer[m_curPos] == '\n')
                    {
                        int length = m_curPos - 1 - startPos;
                        ++m_curPos;
                        if (length == 0)
                            return string.Empty;
            ...
            // If EOB return null else if no \r\n found return remainder of buffer
            if (startPos == m_curPos)
                return null;
            ...
            else
            {
                ...
                byte[] tempBuff = new byte[length];
                Array.Copy(m_buffer, startPos, tempBuff, 0, length);
                return new string(m_utf8Encoding.GetChars(m_buffer), 0, m_curPos);
            }
        }
