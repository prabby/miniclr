...
namespace Microsoft.SPOT.Hardware
{
    public sealed class SPI : IDisposable
    {
        ...
        public void WriteRead(ushort[] writeBuffer, ushort[] readBuffer, int readOffset)
        {
            ...
            if ((writeBuffer == null) || ((readBuffer != null) && (readOffset + readBuffer.Length < writeBuffer.Length)))
            {
                throw new ArgumentException();
            }

            InternalWriteRead(writeBuffer, -1, readBuffer, readOffset);
        }

        public void WriteRead(ushort[] writeBuffer, int writeElemCount, ushort[] readBuffer, int readOffset)
        {
            if (m_disposed)
            {
                throw new ObjectDisposedException();
            ...
            InternalWriteRead(writeBuffer, writeElemCount, readBuffer, readOffset);
        }
        ...
        public void WriteRead(byte[] writeBuffer, int writeElemCount, byte[] readBuffer, int readOffset)
        {
            if (m_disposed)
            {
                throw new ObjectDisposedException();
        ...
        extern private void InternalWriteRead(ushort[] writeBuffer, int writeElemCount, ushort[] readBuffer, int readOffset);

        [MethodImplAttribute(MethodImplOptions.InternalCall)]
        extern private void InternalWriteRead(byte[] writeBuffer, int writeElemCount, byte[] readBuffer, int readOffset);
    }
}
