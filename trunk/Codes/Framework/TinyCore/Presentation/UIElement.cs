...
namespace Microsoft.SPOT.Presentation
{
    public abstract class UIElement : DispatcherObject
    {
        ...
        private static void OnButtonDownThunk(object sender, ButtonEventArgs e) { ((UIElement)sender).OnButtonDown(e); }

        private static void OnPreviewButtonUpThunk(object sender, ButtonEventArgs e) { ((UIElement)sender).OnPreviewButtonUp(e); }

        private static void OnButtonUpThunk(object sender, ButtonEventArgs e) { ((UIElement)sender).OnButtonUp(e); }

        private static void OnGotFocusThunk(object sender, FocusChangedEventArgs e) { ((UIElement)sender).OnGotFocus(e); }
        ...
        private void AddToEventRouteImpl(EventRoute route, RoutedEventArgs args)
        {
            ...
            //
            EventHandlersStore store = _classEventHandlersStore;
            RoutedEvent evt = args._routedEvent;
            for (int repeat = 0; repeat < 2; repeat++)
            {
                if (store != null)
                {
                    ArrayList eventListeners = store[evt];
        ...
        /// </summary>
        protected EventHandlersStore InstanceEventHandlersStore
        {
            get
            {
                if (_instanceEventHandlersStore == null)
                {
                    _instanceEventHandlersStore = new EventHandlersStore();
                }
        ...
        private static EventHandlersStore _classEventHandlersStore;
        private EventHandlersStore _instanceEventHandlersStore;
