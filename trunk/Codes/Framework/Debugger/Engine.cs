...
namespace Microsoft.SPOT.Debugger
{
    ...
    public class Engine : WireProtocol.IControllerHostLocal, IDisposable
    {
        ...
        {
            public const int c_RECONNECT_RETRIES_DEFAULT = 10;
            public const int c_RECONNECT_HARD_TIMEOUT_DEFAULT_MS = 1000;         // one second
            public const int c_RECONNECT_SOFT_TIMEOUT_DEFAULT_MS = 100;          // ten milliseconds
        ...
        public uint GetVirtualMethod(uint md, RuntimeValue obj)
        {
            ...
            cmd.m_md = md;
            cmd.m_obj = obj.ReferenceIdDirect;
        ...
        private bool Deployment_Execute_Incremental(ArrayList assemblies)
        {
        ...
        private bool Deployment_Execute_Full(ArrayList assemblies)
        {
            uint entrypoint;
            uint storageStart;
            uint storageLength;
        ...
        public bool Deployment_Execute(ArrayList assemblies, bool fRebootAfterDeploy, MessageHandler mh)
        {
            ...
            if (this.Capabilities.IncrementalDeployment)
            {
                ...
                fDeployedOK = Deployment_Execute_Incremental(assemblies);
            }
            else
            {
                if (mh != null) mh("Deploying assemblies to device");
                fDeployedOK = Deployment_Execute_Full(assemblies);
            }
