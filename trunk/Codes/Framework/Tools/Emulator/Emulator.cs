...
namespace Microsoft.SPOT.Emulator
{
    public class Emulator : EmulatorComponent, IDisposable
    {
        ...
        private static IEmulator LoadDefaultHAL()
        {
            ...
            halWindowsName.Version = Assembly.GetExecutingAssembly().GetName().Version;
            halWindowsName.SetPublicKey(Assembly.GetExecutingAssembly().GetName().GetPublicKey());

            try
            {
                Assembly halWindowsNativeAssembly = Assembly.Load(halWindowsName);
