...
namespace Ws.SvcImporter
{
    ...
    public class MFSvcImporter
    {
        ...
        public void ParseWsdl(string fileName, List<string> schemaRefs)
        {
            ...
            // Add Import schemas
            foreach(XmlElement element in m_svcDesc.Types.Extensions)
            {
                ...
                if (element.LocalName == "import")
                {
                    ...
                    schemaNs = schemaNs == null ? m_svcDesc.TargetNamespace : schemaNs;
                    ...
                    else
                    {
                        ...
                        reader = XmlReader.Create(schemaLocation, settings);
            ...
            foreach (System.Xml.Schema.XmlSchema wsdlSchema in m_svcDesc.Types.Schemas)
            {
            ...
            foreach (Binding binding in m_svcDesc.Bindings)
            {
                                        ...
                                        if (policyAttrib.LocalName == "Id" && "#" + policyAttrib.Value == attrib.Value)
                                        {
                                            ...
                                            foreach (XmlNode node in policy.ChildNodes)
                                                if (node is XmlElement)
                                                    assertions.Add(new PolicyAssertion(node.LocalName, new Uri(node.NamespaceURI), policyAttrib.Value));
                                        }
                                }
                            }
                    }
                ...
                else
                {
                    ...
                    foreach (XmlElement policy in policies)
                    {
                        ...
                        foreach (XmlNode node in policy.ChildNodes)
                            if (node is XmlElement)
                                assertions.Add(new PolicyAssertion(node.LocalName, new Uri(node.NamespaceURI), policy.Name));
                    }
                }
            }
            return assertions;
