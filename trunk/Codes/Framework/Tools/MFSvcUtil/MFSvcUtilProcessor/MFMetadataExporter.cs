...
namespace Ws.SvcExporter
{
    public class MFMetadataExporter
    {
        ...
        private Binding CreateBinding(PortType portType)
        {
            ...
            extElement = xDoc.CreateElement("wsp", "PolicyReference", "http://schemas.xmlsoap.org/ws/2004/09/policy");
            attrib = xDoc.CreateAttribute("URI");
            attrib.Value = "#" + portType.Name + "Policy";
            extElement.Attributes.Append(attrib);
            attrib = xDoc.CreateAttribute("wsdl", "required", "http://schemas.xmlsoap.org/wsdl/");
            attrib.Value = "true";
            extElement.Attributes.Append(attrib);
            binding.Extensions.Add(extElement);
        ...
        void AddPolicyBindingElement(ServiceDescription serviceDesc, bool hasEvents, bool hasOptimizedMimeContent)
        {
            ...
            XmlDocument xDoc = new XmlDocument();
            XmlElement policy = xDoc.CreateElement("wsp", "Policy", policyNs);
            XmlAttribute policyAttrib = (XmlAttribute)xDoc.CreateAttribute("wsu", "Id", null);
            policyAttrib.Value = serviceDesc.Name + "Policy";
            policy.Attributes.Append(policyAttrib);
            policy.AppendChild(xDoc.CreateElement("wsdp", "profile", dpwsNs));
            if (hasEvents)
            {
                policy.AppendChild(xDoc.CreateElement("wsdp", "PushDelivery", dpwsNs));
                policy.AppendChild(xDoc.CreateElement("wsdp", "DurationExpiration", dpwsNs));
                policy.AppendChild(xDoc.CreateElement("wsdp", "ActionFilter", dpwsNs));
            }
            if (hasOptimizedMimeContent)
            {
                policy.AppendChild(xDoc.CreateElement("wsdp", "OptimizedMimeSerialization", dpwsNs));
            }
