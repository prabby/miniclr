...
static HRESULT Debugging_Thread_Create_Helper( CLR_RT_MethodDef_Index& md, CLR_RT_Thread*& th, CLR_UINT32 pid )
{
    ...
    TINYCLR_CHECK_HRESULT(g_CLR_RT_ExecutionEngine.NewThread( th, ref.DereferenceDelegate(), 0 ));
...
static bool SetBlockHelper( CLR_RT_HeapBlock* blk, CLR_DataType dt, CLR_UINT8* builtinValue )
{
    ...
    if(blk)
    {
        CLR_DataType                 dtDst = blk->DataType();
        ...
        else
        {
            ...
            {
                _ASSERTE(c_CLR_RT_DataTypeLookup[ blk->DataType() ].m_flags & CLR_RT_DataTypeLookup::c_Numeric);
