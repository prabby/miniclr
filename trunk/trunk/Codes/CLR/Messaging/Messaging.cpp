...
bool CLR_Messaging::TransmitMessage( const WP_Message* msg, bool fQueue )
{
    ...
    if(fQueue && (msg->m_header.m_flags & WP_Flags::c_NoCaching) == 0)
    {
        ...
        {
            if(msg->m_header.m_flags & WP_Flags::c_NonCritical)
            {
                //
        ...
        if(lst)
        {
            CLR_UINT32 len = sizeof(CachedMessage) + msg->m_header.m_size;
            ...
            if(cache)
            {
                ...
                if(msg->m_header.m_size && msg->m_payload)
                {
                    ...
                    memcpy( cache->m_message.m_payload, msg->m_payload, msg->m_header.m_size );
                }
                else
                {
