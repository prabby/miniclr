...
HRESULT CLR_RT_HeapBlock::LoadFromReference( CLR_RT_HeapBlock& ref )
{
    ...
    if(dt == DATATYPE_ARRAY_BYREF)
    {
        ...
        if(!array->m_fReference)
        {
            ...
            if     (size == 4) { low = ((CLR_UINT32*)src)[ 0 ];                                 }
            else if(size == 8) { low = ((CLR_UINT32*)src)[ 0 ]; high = ((CLR_UINT32*)src)[ 1 ]; }
            else if(size == 1) { low = ((CLR_UINT8 *)src)[ 0 ];                                 }
            else               { low = ((CLR_UINT16*)src)[ 0 ];                                 }

            ((CLR_UINT32*)&NumericByRef())[ 0 ] = low;
            ((CLR_UINT32*)&NumericByRef())[ 1 ] = high;
...
HRESULT CLR_RT_HeapBlock::StoreToReference( CLR_RT_HeapBlock& ref, int size )
{
    ...
    if(dt == DATATYPE_ARRAY_BYREF)
    {
        ...
        if(!array->m_fReference)
        {
            ...
            CLR_UINT32 low  = ((CLR_UINT32*)&obj->NumericByRef())[ 0 ];
            CLR_UINT32 high = ((CLR_UINT32*)&obj->NumericByRef())[ 1 ];

            if     (sizeArray == 4) { ((CLR_UINT32*)dst)[ 0 ] = (CLR_UINT32)low;                                             }
            else if(sizeArray == 8) { ((CLR_UINT32*)dst)[ 0 ] = (CLR_UINT32)low; ((CLR_UINT32*)dst)[ 1 ] = (CLR_UINT32)high; }
            else if(sizeArray == 1) { ((CLR_UINT8 *)dst)[ 0 ] = (CLR_UINT8 )low;                                             }
            else                    { ((CLR_UINT16*)dst)[ 0 ] = (CLR_UINT16)low;                                             }
...
HRESULT CLR_RT_HeapBlock::PerformBoxing( const CLR_RT_TypeDef_Instance& cls )
{
...

    {
        ...
        else if(dt == DATATYPE_OBJECT)
        {
            ...
            {
                TINYCLR_SET_AND_LEAVE(CLR_E_WRONG_TYPE); // Don't box twice...
            }
...
{
    NATIVE_PROFILE_CLR_CORE();
    CLR_INT32  upperWordLeft  = (left  >> 32);
    CLR_INT32  upperWordRight = (right >> 32);
    CLR_UINT32 lowerWordLeft  = (left  & 0xFFFFFFFF);
    CLR_UINT32 lowerWordRight = (right & 0xFFFFFFFF);

    int res = CompareValues_Numeric( upperWordLeft, upperWordRight );

    return res ? res : CompareValues_Numeric( lowerWordLeft, lowerWordRight );
...
static int CompareValues_Numeric( const CLR_UINT64 left, const CLR_UINT64 right )
{
    ...
    CLR_UINT32 upperWordLeft  = (left  >> 32);
    CLR_UINT32 upperWordRight = (right >> 32);
    CLR_UINT32 lowerWordLeft  = (left  & 0xFFFFFFFF);
    CLR_UINT32 lowerWordRight = (right & 0xFFFFFFFF);
