...
HRESULT Library_corlib_native_System_Type::get_DeclaringType___SystemType( CLR_RT_StackFrame& stack )
{
    ...
    TINYCLR_CHECK_HRESULT(Library_corlib_native_System_RuntimeType::GetTypeDescriptor( stack.Arg0(), td ));
...
HRESULT Library_corlib_native_System_Type::IsInstanceOfType___BOOLEAN__OBJECT( CLR_RT_StackFrame& stack )
{
    ...
    if(stack.Arg0().DataType() != DATATYPE_REFLECTION) TINYCLR_SET_AND_LEAVE(CLR_E_WRONG_TYPE);

    TINYCLR_CHECK_HRESULT(descTarget.InitializeFromReflection( stack.Arg0().ReflectionDataConst() ));
...
HRESULT Library_corlib_native_System_Type::GetTypeFromHandle___STATIC__SystemType__SystemRuntimeTypeHandle( CLR_RT_StackFrame& stack )
{
    ...
    stack.PushValueAndAssign( pThis ); // RuntimeTypeHandle and Type have the same representation.    
...
HRESULT Library_corlib_native_System_Type::GetFields( CLR_RT_StackFrame& stack, LPCSTR szText, CLR_UINT32 bindingFlags, bool fAllMatches )
{
    ...
    TINYCLR_CHECK_HRESULT(Library_corlib_native_System_RuntimeType::GetTypeDescriptor( stack.Arg0(), tdArg ));
            ...
            do
            {
                ...
                for(i=0; i<iTot; i++, fd++)
                {
                    ...
                    else if(pass == 1)
                    {
                        ...
                        TINYCLR_CHECK_HRESULT(elem->SetReflection( idx ));
                    }
