...
extern "C"
{
    void rtp_rand_seed(const void *seed, int length);
}
...
static void GenerateNewSslSeed()
{
    ...
    rtp_rand_seed(signature, sizeof(signature));
...
HRESULT Library_spot_net_security_native_Microsoft_SPOT_Net_Security_SslNative::UpdateCertificates___STATIC__VOID__I4__MicrosoftSPOTNetSecurityX509Certificate__SZARRAY_MicrosoftSPOTNetSecurityX509Certificate( CLR_RT_StackFrame& stack )
{
    ...
    arrCert    = hbCert[ Library_spot_net_security_native_Microsoft_SPOT_Net_Security_X509Certificate::FIELD__m_certificate ].DereferenceArray(); FAULT_ON_NULL(arrCert);

    sslCert    = arrCert->GetFirstElement();

    hbPwd      = hbCert[ Library_spot_net_security_native_Microsoft_SPOT_Net_Security_X509Certificate::FIELD__m_password ].Dereference(); FAULT_ON_NULL(hbPwd);
    ...
    for(i=0; i<(int)arrCA->m_numOfElements; i++)
    {
        ...
        arrCert = hbCert[ Library_spot_net_security_native_Microsoft_SPOT_Net_Security_X509Certificate::FIELD__m_certificate ].DereferenceArray();

        sslCert = arrCert->GetFirstElement();

        hbPwd      = hbCert[ Library_spot_net_security_native_Microsoft_SPOT_Net_Security_X509Certificate::FIELD__m_password ].Dereference();
...
HRESULT Library_spot_net_security_native_Microsoft_SPOT_Net_Security_SslNative::InitHelper( CLR_RT_StackFrame& stack, bool isServer )
{
    ...
    if(hbCert != NULL)
    {
        ...
        CLR_RT_HeapBlock *hbPwd = hbCert[ Library_spot_net_security_native_Microsoft_SPOT_Net_Security_X509Certificate::FIELD__m_password ].Dereference(); FAULT_ON_NULL(hbPwd);
