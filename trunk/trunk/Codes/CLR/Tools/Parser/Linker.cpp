...
bool WatchAssemblyBuilder::Linker::GenerateSignature( MetaData::Reparser::TypeSignature* sig )
{
    *m_tmpSigPtr++ = sig->m_opt;

    switch(sig->m_opt)
    {
    case DATATYPE_BYREF    : return GenerateSignature     ( sig->m_sub   );
    case DATATYPE_VALUETYPE: return GenerateSignatureToken( sig->m_token );
    case DATATYPE_CLASS    : return GenerateSignatureToken( sig->m_token );
    case DATATYPE_SZARRAY  : return GenerateSignature     ( sig->m_sub   );
    }
...
bool WatchAssemblyBuilder::Linker::GenerateSignature( MetaData::Reparser::Attribute* ca )
{
    wprintf( L"ERROR: Attributes not supported yet!" );
    return false;
}
...
bool WatchAssemblyBuilder::Linker::GenerateSignatureToken( MetaData::Reparser::BaseToken* ptr )
{
    mdToken tk = ptr->m_tk;
    ...
    {
    case TBL_TypeDef : token = token << 2 | 0x00; break;
    case TBL_TypeRef : token = token << 2 | 0x01; break;
    case TBL_TypeSpec: token = token << 2 | 0x02; break;
    default          : REPORT_FAILURE();
    }
...
bool WatchAssemblyBuilder::Linker::AllocSignatureForField( MetaData::Reparser::TypeSignature* sig, CLR_SIG& idx )
{
    PrepareSignature();
...
bool WatchAssemblyBuilder::Linker::AllocSignatureForMethod( MetaData::Reparser::MethodSignature& sig, CLR_SIG& idx )
{
    PrepareSignature();
    ...
    *m_tmpSigPtr++ = sig.m_flags;
    ...
    for(MetaData::Reparser::TypeSignatureIter it = sig.m_lstParams.begin(); it != sig.m_lstParams.end(); it++)
    {
        MetaData::Reparser::TypeSignature& ptr = *it;

        if(!GenerateSignature( &ptr )) return false;
    }
...
bool WatchAssemblyBuilder::Linker::AllocSignatureForAttribute( MetaData::Reparser::Attribute* ca, CLR_SIG& idx )
{
    wprintf( L"ERROR: Attributes not supported yet!" );
    return false;
}
...
bool WatchAssemblyBuilder::Linker::AllocSignatureForTypeSpec( MetaData::Reparser::TypeSignature* sig, CLR_SIG& idx )
{
    PrepareSignature();
...
bool WatchAssemblyBuilder::Linker::AllocSignatureForLocalVar( MetaData::Reparser::LocalVarSignature& sig, CLR_SIG& idx )
{
    if(sig.m_lstVars.size() == 0) { idx = CLR_EmptyIndex; return true; }

    PrepareSignature();

    for(MetaData::Reparser::TypeSignatureIter it = sig.m_lstVars.begin(); it != sig.m_lstVars.end(); it++)
    {
        MetaData::Reparser::TypeSignature& sig2 = *it;
...
bool WatchAssemblyBuilder::Linker::AllocSignatureForInterfaces( MetaData::Reparser::BaseTokenPtrList& lst, CLR_SIG& idx )
{
    if(lst.size() == 0) { idx = CLR_EmptyIndex; return true; }

    PrepareSignature();

    if(!GenerateSignatureData8( (CLR_UINT8)lst.size() )) return false;

    for(MetaData::Reparser::BaseTokenPtrIter it = lst.begin(); it != lst.end(); it++)
    {
        if(!GenerateSignatureToken( *it )) return false;
    }
...
CLR_IDX WatchAssemblyBuilder::Linker::EncodeTypeRefOrDef( MetaData::Reparser::BaseToken* ptr )
{
    if(!ptr)
    {
        return CLR_EmptyIndex;
    ...
    CLR_IDX val = m_lookupIDs[ptr->m_tk];

    return (ptr->GetTableOfInstance() == TBL_TypeRef) ? val | 0x8000 : val;
}
...
CLR_IDX WatchAssemblyBuilder::Linker::EncodeToken( MetaData::Reparser::BaseToken* ptr )
{
    if(!ptr)
    {
        return CLR_EmptyIndex;
...
HRESULT WatchAssemblyBuilder::Linker::Reprocess( MetaData::Reparser::Assembly& rpr )
{
    TINYCLR_HEADER();
    ...
    {
        MetaData::Reparser::BaseTokenPtrList order;

        TINYCLR_CHECK_HRESULT(ReprocessAssemblyRef(       ));
        TINYCLR_CHECK_HRESULT(ReprocessTypeRef    (       ));
        TINYCLR_CHECK_HRESULT(ReprocessFieldRef   (       ));
        TINYCLR_CHECK_HRESULT(ReprocessMethodRef  (       ));
        TINYCLR_CHECK_HRESULT(ReprocessTypeDef    ( order ));
        TINYCLR_CHECK_HRESULT(ReprocessTypeSpec   (       ));
        TINYCLR_CHECK_HRESULT(ReprocessAttribute  (       ));
        TINYCLR_CHECK_HRESULT(ReprocessResource   (       ));
        TINYCLR_CHECK_HRESULT(ReprocessUserString (       ));

        for(MetaData::Reparser::BaseTokenPtrIter itOrder = order.begin(); itOrder != order.end(); itOrder++)
        {
            MetaData::Reparser::TypeDef* td = itOrder->CastTo( MetaData::Reparser::TypeDef() );

            TINYCLR_CHECK_HRESULT(ReprocessTypeDef_ByteCode( td ));
        }
    }
...
HRESULT WatchAssemblyBuilder::Linker::ReprocessAssemblyRef()
{
    TINYCLR_HEADER();

    REPARSER_TABLE_ENUM_BEGIN(m_rpr,MetaData::Reparser::AssemblyRef)
    {
        m_lookupIDs[rec->m_tk] = CLR_TkFromType( TBL_AssemblyRef, (int)m_tableAssemblyRef.GetPos() );

        CLR_RECORD_ASSEMBLYREF* dst = m_tableAssemblyRef.Alloc( 1 ); if(dst == NULL) REPORT_NO_MEMORY();

        if(!AllocString( rec->m_name, dst->name, false )) REPORT_NO_MEMORY();

        dst->version = rec->m_version;
    }
...
HRESULT WatchAssemblyBuilder::Linker::ReprocessTypeRef()
{
    TINYCLR_HEADER();

    MetaData::Reparser::BaseTokenPtrConstIter itOrder;
    MetaData::Reparser::BaseTokenPtrList      order;
    MetaData::mdTokenSet                      resolved;
    ...
    // This is good, because the layout will be ordered and no multiple passes will be required on the runtime end.
    //
    REPARSER_TABLE_ENUM_BEGIN(m_rpr,MetaData::Reparser::TypeRef)
    {
        TINYCLR_CHECK_HRESULT(ResolveTypeRef( *rec, order, resolved ));
    }
    REPARSER_TABLE_ENUM_END();
    ...
    for(itOrder = order.begin(); itOrder != order.end(); itOrder++)
    {
        MetaData::Reparser::TypeRef* tr = itOrder->CastTo( MetaData::Reparser::TypeRef() );

        m_lookupIDs[tr->m_tk] = CLR_TkFromType( TBL_TypeRef, (CLR_UINT32)m_tableTypeRef.GetPos() );
    ...
    for(itOrder = order.begin(); itOrder != order.end(); itOrder++)
    {
        ...
        CLR_RECORD_TYPEREF* dst = m_tableTypeRef.GetRecordAt( CLR_DataFromTk( m_lookupIDs[tr->m_tk] ) );

        if(!AllocString( tr->m_name, tr->m_nameSpace, dst->name, dst->nameSpace )) REPORT_NO_MEMORY();
...
HRESULT WatchAssemblyBuilder::Linker::ReprocessFieldRef()
{
    TINYCLR_HEADER();

    REPARSER_TABLE_ENUM_BEGIN(m_rpr,MetaData::Reparser::FieldRef)
    {
        m_lookupIDs[rec->m_tk] = CLR_TkFromType( TBL_FieldRef, (CLR_UINT32)m_tableFieldRef.GetPos() );

        CLR_RECORD_FIELDREF* dst = m_tableFieldRef.Alloc( 1 ); if(dst == NULL) REPORT_NO_MEMORY();

        if(!AllocString           (  rec->m_name, dst->name, false )) REPORT_NO_MEMORY();
        if(!AllocSignatureForField( &rec->m_sig , dst->sig         )) REPORT_NO_MEMORY();

        if(!rec->m_container) REPORT_NO_MEMORY();
        CLR_UINT32 tk = m_lookupIDs[ rec->m_container->m_tk ]; if(CLR_TypeFromTk(tk) != TBL_TypeRef) REPORT_NO_MEMORY();
        dst->container = CLR_DataFromTk( tk );
    }
    REPARSER_TABLE_ENUM_END();
...
HRESULT WatchAssemblyBuilder::Linker::ReprocessMethodRef()
{
    TINYCLR_HEADER();

    REPARSER_TABLE_ENUM_BEGIN(m_rpr,MetaData::Reparser::MethodRef)
    {
        m_lookupIDs[rec->m_tk] = CLR_TkFromType( TBL_MethodRef, (CLR_UINT32)m_tableMethodRef.GetPos() );

        CLR_RECORD_METHODREF* dst = m_tableMethodRef.Alloc( 1 ); if(dst == NULL) REPORT_NO_MEMORY();

        if(!AllocString            ( rec->m_name, dst->name, false )) REPORT_NO_MEMORY();
        if(!AllocSignatureForMethod( rec->m_sig , dst->sig         )) REPORT_NO_MEMORY();
...
HRESULT WatchAssemblyBuilder::Linker::ReprocessTypeDef( MetaData::Reparser::BaseTokenPtrList& order )
{
    TINYCLR_HEADER();
    ...
    // This is good, because the layout will be ordered and no multiple passes will be required on the runtime end.
    //
    REPARSER_TABLE_ENUM_BEGIN(m_rpr,MetaData::Reparser::TypeDef)
    {
        TINYCLR_CHECK_HRESULT(ResolveTypeDef( *rec, order, resolved ));
    }
    REPARSER_TABLE_ENUM_END();
    ...
    for(itOrder = order.begin(); itOrder != order.end(); itOrder++)
    {
        ...
        m_lookupIDs[td->m_tk] = CLR_TkFromType( TBL_TypeDef, (CLR_UINT32)m_tableTypeDef.GetPos() );
    ...
    for(itOrder = order.begin(); itOrder != order.end(); itOrder++)
    {
        ...
        TINYCLR_CHECK_HRESULT(ReprocessTypeDef( td ));
    }
...
HRESULT WatchAssemblyBuilder::Linker::ReprocessTypeDef( MetaData::Reparser::TypeDef* td )
{
    TINYCLR_HEADER();

    MetaData::Reparser::BaseTokenPtrIter itField;
    MetaData::Reparser::BaseTokenPtrIter itMethod;


    CLR_RECORD_TYPEDEF* dst = m_tableTypeDef.GetRecordAt( CLR_DataFromTk( m_lookupIDs[td->m_tk] ) );

    if(!AllocString( td->m_name, td->m_nameSpace, dst->name, dst->nameSpace )) REPORT_NO_MEMORY();

    dst->extends       = EncodeTypeRefOrDef( td->m_extends       );
    dst->enclosingType = EncodeToken       ( td->m_enclosingType );

    dst->flags         = td->m_flags;
    dst->dataType      = td->m_dataType;
    ...
    //
    dst->sFields_First = (CLR_IDX)m_tableFieldDef.GetPos();
    for(itField = td->m_fields_Static.begin(); itField != td->m_fields_Static.end(); itField++)
    {
        MetaData::Reparser::FieldDef* fd = itField->CastTo( MetaData::Reparser::FieldDef() );

        TINYCLR_CHECK_HRESULT(ReprocessFieldDef( td, dst, fd ));
    }
    TINYCLR_CHECK_HRESULT(m_tableFieldDef.VerifyRange( dst->sFields_First, 0x00FF, L"number", L"static fields" ));
    dst->sFields_Num = (CLR_UINT8)((CLR_IDX)m_tableFieldDef.GetPos() - dst->sFields_First);
    ...
    //
    dst->iFields_First = (CLR_UINT32)m_tableFieldDef.GetPos();
    for(itField = td->m_fields_Instance.begin(); itField != td->m_fields_Instance.end(); itField++)
    {
        MetaData::Reparser::FieldDef* fd = itField->CastTo( MetaData::Reparser::FieldDef() );
    ...
    // Virtual Methods.
    //
    for(itMethod = td->m_methods_Virtual.begin(); itMethod != td->m_methods_Virtual.end(); itMethod++)
    {
        MetaData::Reparser::MethodDef* md = itMethod->CastTo( MetaData::Reparser::MethodDef() );

        TINYCLR_CHECK_HRESULT(ReprocessMethodDef( td, dst, md ));
    }
    TINYCLR_CHECK_HRESULT(m_tableMethodDef.VerifyRange( lastMethod, 0xFF, L"number", L"virtual methods" ));
    dst->vMethods_Num = (CLR_UINT8)((CLR_IDX)m_tableMethodDef.GetPos() - lastMethod); lastMethod += dst->vMethods_Num;
    ...
    // Instance Methods.
    //
    for(itMethod = td->m_methods_Instance.begin(); itMethod != td->m_methods_Instance.end(); itMethod++)
    {
        MetaData::Reparser::MethodDef* md = itMethod->CastTo( MetaData::Reparser::MethodDef() );
    ...
    // Static Methods.
    //
    for(itMethod = td->m_methods_Static.begin(); itMethod != td->m_methods_Static.end(); itMethod++)
    {
        MetaData::Reparser::MethodDef* md = itMethod->CastTo( MetaData::Reparser::MethodDef() );
    ...
    if(FAILED(hr))
    {
        wprintf( L"Failed compiling type '%S.%S'\n", td->m_nameSpace.c_str(), td->m_name.c_str() );
    }
...
HRESULT WatchAssemblyBuilder::Linker::ReprocessTypeDef_ByteCode( MetaData::Reparser::TypeDef* td )
{
    TINYCLR_HEADER();
    ...
    for(itMethod = td->m_methods_Virtual.begin(); itMethod != td->m_methods_Virtual.end(); itMethod++)
    {
        ...
        TINYCLR_CHECK_HRESULT(ReprocessMethodDef_ByteCode( td, dst, md ));
    }
...
HRESULT WatchAssemblyBuilder::Linker::ReprocessFieldDef( MetaData::Reparser::TypeDef* td, CLR_RECORD_TYPEDEF* tdDst, MetaData::Reparser::FieldDef* fd )
{
    TINYCLR_HEADER();

    m_lookupIDs[fd->m_tk] = CLR_TkFromType( TBL_FieldDef, (CLR_UINT32)m_tableFieldDef.GetPos() );

    CLR_RECORD_FIELDDEF* dst = m_tableFieldDef.Alloc( 1 ); if(dst == NULL) REPORT_NO_MEMORY();


    if(!AllocString           (  fd->m_name, dst->name, false )) REPORT_NO_MEMORY();
    if(!AllocSignatureForField( &fd->m_sig , dst->sig         )) REPORT_NO_MEMORY();

    dst->flags = fd->m_flags;

    if(fd->m_defaultValue.size() > 0)
    {
        CLR_RT_Buffer valueWithSize;
        CLR_UINT16    len = (CLR_UINT16)fd->m_defaultValue.size();

        valueWithSize.resize( len + sizeof(len) );

        memcpy( &valueWithSize[0            ], &len                  , sizeof(len) );
        memcpy( &valueWithSize[0+sizeof(len)], &fd->m_defaultValue[0],        len  );
    ...
    if(FAILED(hr))
    {
        wprintf( L"Failed compiling field '%S.%S.%S'\n", td->m_nameSpace.c_str(), td->m_name.c_str(), fd->m_name.c_str() );
    }
...
HRESULT WatchAssemblyBuilder::Linker::ReprocessMethodDef( MetaData::Reparser::TypeDef* td, CLR_RECORD_TYPEDEF* tdDst, MetaData::Reparser::MethodDef* md )
{
    TINYCLR_HEADER();

    //--//

    m_lookupIDs[md->m_tk] = CLR_TkFromType( TBL_MethodDef, (CLR_UINT32)m_tableMethodDef.GetPos() );

    CLR_RECORD_METHODDEF* dst = m_tableMethodDef.Alloc( 1 ); if(dst == NULL) REPORT_NO_MEMORY();

    dst->flags           = md->m_flags;
    dst->retVal          = md->m_retVal;
    dst->numArgs         = md->m_numArgs;
    dst->numLocals       = md->m_numLocals;
    dst->lengthEvalStack = md->m_lengthEvalStack;

    if(!AllocString              ( md->m_name  , dst->name  , false )) REPORT_NO_MEMORY();
    if(!AllocSignatureForMethod  ( md->m_sig   , dst->sig           )) REPORT_NO_MEMORY();
    if(!AllocSignatureForLocalVar( md->m_locals, dst->locals        )) REPORT_NO_MEMORY();
    ...
    if(FAILED(hr))
    {
        wprintf( L"Failed compiling method '%S.%S.%S'\n", td->m_nameSpace.c_str(), td->m_name.c_str(), md->m_name.c_str() );
    }
...
HRESULT WatchAssemblyBuilder::Linker::ProcessMethodDef_ByteCode( MetaData::TypeDef& td, CLR_RECORD_TYPEDEF* tdDst, MetaData::MethodDef& md, CLR_UINT32 mode )
{
    ...
    else if(md.m_byteCode.m_opcodes.size() > 0)
    {
        ...
        TINYCLR_CHECK_HRESULT(md.m_byteCode.GenerateOldIL( code        ));
...
HRESULT WatchAssemblyBuilder::Linker::ReprocessMethodDef_ByteCode( MetaData::Reparser::TypeDef* td, CLR_RECORD_TYPEDEF* tdDst, MetaData::Reparser::MethodDef* md )
{
    TINYCLR_HEADER();

    CLR_RECORD_METHODDEF* dst = m_tableMethodDef.GetRecordAt( CLR_DataFromTk( m_lookupIDs[md->m_tk] ) );

    if(md->m_byteCode.size() == 0)
    {
        dst->RVA = CLR_EmptyIndex;
    }
    else
    {
        const BYTE* byteCodeSrc   = &md->m_byteCode[0];
        size_t      byteCodeLen   =  md->m_byteCode.size();
        size_t      numExceptions =  md->m_eh.size();
        ...
        {
            CLR_PMETADATA pStart = byteCodeDst;
            CLR_PMETADATA pEnd   = byteCodeDst + byteCodeLen;
            ...
            while(ip < pEnd)
            {
                ...
                if(IsOpParamToken( c_CLR_RT_OpcodeLookup[op].m_opParam ))
                {
                    ...
                    MetaData::Reparser::BaseToken* ptrTk = md->m_tokens[index];
                    mdToken                        tk    = this->m_lookupIDs[ ptrTk->m_tk ];

                    switch(c_CLR_RT_OpcodeLookup[op].m_opParam)
                    {
                    case CLR_OpcodeParam_Field : TINYCLR_CHECK_HRESULT( CLR_CompressFieldToken ( tk ) ); TINYCLR_WRITE_UNALIGNED_UINT16( ip, (CLR_UINT16)tk ); break;
                    case CLR_OpcodeParam_Method: TINYCLR_CHECK_HRESULT( CLR_CompressMethodToken( tk ) ); TINYCLR_WRITE_UNALIGNED_UINT16( ip, (CLR_UINT16)tk ); break;
                    case CLR_OpcodeParam_String: TINYCLR_CHECK_HRESULT( CLR_CompressStringToken( tk ) ); TINYCLR_WRITE_UNALIGNED_UINT16( ip, (CLR_UINT16)tk ); break;
                    case CLR_OpcodeParam_Type  : TINYCLR_CHECK_HRESULT( CLR_CompressTypeToken  ( tk ) ); TINYCLR_WRITE_UNALIGNED_UINT16( ip, (CLR_UINT16)tk ); break;
                    default                    :                                                         TINYCLR_WRITE_UNALIGNED_UINT32( ip, (CLR_UINT32)tk ); break;
                    }
                }
                else
                {
        ...
        if(numExceptions > 0)
        {
            ...
            size_t align = m_tableByteCode.Size() % sizeof(CLR_UINT32);
            if(align)
            {
                align = sizeof(CLR_UINT32) - align;

                byteCodeDst = m_tableByteCode.Alloc( align ); if(byteCodeDst == NULL) REPORT_NO_MEMORY();

                memset( byteCodeDst, CEE_NOP, align );
            }

            for(MetaData::Reparser::MethodDef::EhIter it = md->m_eh.begin(); it != md->m_eh.end(); it++)
            {
                MetaData::Reparser::MethodDef::Eh& ehSrc = *it;
                CLR_RECORD_EH                      ehDst; TINYCLR_CLEAR(ehDst);

                ehDst.mode         =                     ehSrc.m_mode          ;
                ehDst.classToken   = EncodeTypeRefOrDef( ehSrc.m_classToken   );
                ehDst.tryStart     =                     ehSrc.m_tryStart      ;
                ehDst.tryEnd       =                     ehSrc.m_tryEnd        ;
                ehDst.handlerStart =                     ehSrc.m_handlerStart  ;
                ehDst.handlerEnd   =                     ehSrc.m_handlerEnd    ;

                lookupEh.Queue( ehDst );
            }
...
HRESULT WatchAssemblyBuilder::Linker::ReprocessAttribute()
{
    TINYCLR_HEADER();

    REPARSER_TABLE_ENUM_BEGIN(m_rpr,MetaData::Reparser::Attribute)
    {
        wprintf( L"ERROR: Attributes not supported yet!" );
        REPORT_NO_MEMORY();
    }
...
HRESULT WatchAssemblyBuilder::Linker::ReprocessResource()
{
    TINYCLR_HEADER();
...
HRESULT WatchAssemblyBuilder::Linker::ReprocessTypeSpec()
{
    TINYCLR_HEADER();

    REPARSER_TABLE_ENUM_BEGIN(m_rpr,MetaData::Reparser::TypeSpec)
    {
        m_lookupIDs[rec->m_tk] = CLR_TkFromType( TBL_TypeSpec, (CLR_UINT32)m_tableTypeSpec.GetPos() );

        CLR_RECORD_TYPESPEC* dst = m_tableTypeSpec.Alloc( 1 ); if(dst == NULL) REPORT_NO_MEMORY();

        if(!AllocSignatureForTypeSpec( &rec->m_sig, dst->sig )) REPORT_NO_MEMORY();
    }
    REPARSER_TABLE_ENUM_END();
...
HRESULT WatchAssemblyBuilder::Linker::ReprocessUserString()
{
    TINYCLR_HEADER();

    CLR_STRING idx;

    REPARSER_TABLE_ENUM_BEGIN(m_rpr,MetaData::Reparser::String)
    {
        if(!AllocString( rec->m_value, idx, true )) REPORT_NO_MEMORY();

        m_lookupIDs[rec->m_tk] = CLR_TkFromType( TBL_Strings, idx );
    }
    REPARSER_TABLE_ENUM_END();
...
HRESULT WatchAssemblyBuilder::Linker::ResolveTypeRef( MetaData::Reparser::TypeRef& tr, MetaData::Reparser::BaseTokenPtrList& order, MetaData::mdTokenSet& resolved )
{
    TINYCLR_HEADER();
    ...
    {
        MetaData::Reparser::TypeRef* scope = tr.m_scope.CastTo( MetaData::Reparser::TypeRef() );

        if(scope)
        {
            if(MetaData::IsTokenPresent( resolved, scope->m_tk ) == false)
            {
                TINYCLR_CHECK_HRESULT(ResolveTypeRef( *scope, order, resolved ));
            }
        }
    }

    order   .push_back( &tr      );
    resolved.insert   (  tr.m_tk );
...
HRESULT WatchAssemblyBuilder::Linker::ResolveTypeDef( MetaData::Reparser::TypeDef& td, MetaData::Reparser::BaseTokenPtrList& order, MetaData::mdTokenSet& resolved )
{
    TINYCLR_HEADER();
    ...
    // Resolve enclosing class.
    //
    if(!!td.m_enclosingType) // Nested class.
    {
        MetaData::Reparser::TypeDef* enclosingType = td.m_enclosingType.CastTo( MetaData::Reparser::TypeDef() );

        if(enclosingType)
        {
            if(MetaData::IsTokenPresent( resolved, enclosingType->m_tk ) == false)
            {
                TINYCLR_CHECK_HRESULT(ResolveTypeDef( *enclosingType, order, resolved ));
            }
        }
    }
    ...
    if(!!td.m_extends)
    {
        MetaData::Reparser::TypeDef* extends = td.m_extends.CastTo( MetaData::Reparser::TypeDef() );

        if(extends)
        {
            if(MetaData::IsTokenPresent( resolved, extends->m_tk ) == false)
            {
                TINYCLR_CHECK_HRESULT(ResolveTypeDef( *extends, order, resolved ));
            }
        }
    }
    ...
    // Resolve interfaces.
    //
    for(MetaData::Reparser::BaseTokenPtrConstIter it = td.m_interfaces.begin(); it != td.m_interfaces.end(); it++)
    {
        MetaData::Reparser::TypeDef* itf = it->CastTo( MetaData::Reparser::TypeDef() );

        if(itf)
        {
            if(MetaData::IsTokenPresent( resolved, itf->m_tk ) == false)
            {
                TINYCLR_CHECK_HRESULT(ResolveTypeDef( *itf, order, resolved ));
            }
        }
    }
...
HRESULT WatchAssemblyBuilder::Linker::Generate( CQuickRecord<BYTE>& buf, bool patch_fReboot, bool patch_fSign, std::wstring* patch_szNative )
{
    ...
    {
        header.version = m_rpr->m_version;

        if(!AllocString( m_rpr->m_name, header.assemblyName, false )) REPORT_NO_MEMORY();

        for(SIZE_T i=0; i<m_tableMethodDef.GetPos(); i++)
        {
            const CLR_RECORD_METHODDEF* md = m_tableMethodDef.GetRecordAt( i );

            if(md->flags & CLR_RECORD_METHODDEF::MD_Patched) header.numOfPatchedMethods++;
        }

        header.flags |= CLR_RECORD_ASSEMBLY::c_Flags_Patch;
    }
...
HRESULT WatchAssemblyBuilder::Linker::DumpDownloads( LPCWSTR szFileNamePE )
{
    ...
    {
        IXMLDOMNodePtr pNodePatch;
        std::wstring   strPath;

        TINYCLR_CHECK_HRESULT(xml.CreateNode( L"Patch", &pNodePatch ));

        TINYCLR_CHECK_HRESULT(xml.PutValue( L"Name", _variant_t( m_rpr->m_assemblyOrig->m_name.c_str() ), fFound, pNodePatch ));

        TINYCLR_CHECK_HRESULT(local_DumpVersion( xml, pNodePatch, L"Version_Original", m_rpr->m_assemblyOrig   ->m_version ));
        TINYCLR_CHECK_HRESULT(local_DumpVersion( xml, pNodePatch, L"Version_Fixed"   , m_rpr->m_assemblyPatched->m_version ));

        TINYCLR_CHECK_HRESULT(GetFullPath ( m_rpr->m_assemblyOrig   ->m_assemblyFile, &strPath, NULL               ));
        TINYCLR_CHECK_HRESULT(xml.PutValue( L"Original",                               strPath, fFound, pNodePatch ));

        TINYCLR_CHECK_HRESULT(GetFullPath ( m_rpr->m_assemblyPatched->m_assemblyFile, &strPath, NULL               ));
        TINYCLR_CHECK_HRESULT(xml.PutValue( L"Fixed",                                  strPath, fFound, pNodePatch ));

        TINYCLR_CHECK_HRESULT(GetFullPath ( szFileNamePE,                             &strPath, NULL               ));
        TINYCLR_CHECK_HRESULT(xml.PutValue( L"Diff",                                   strPath, fFound, pNodePatch ));
    }
    else
    {
        TINYCLR_SET_AND_LEAVE(CLR_E_FAIL);
