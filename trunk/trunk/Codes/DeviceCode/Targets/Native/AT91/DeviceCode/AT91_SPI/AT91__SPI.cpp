...
BOOL AT91_SPI_Driver::Initialize()
{
	...
	pmc.PMC_PCER |= 0x2000; //enable SPI0
#else	
    pmc.PMC_PCER |= 0x1000; //enable SPI0
#endif
	...
	CPU_GPIO_EnableInputPin(AT91_SPI0_NSS, FALSE, NULL, GPIO_INT_NONE, RESISTOR_PULLUP);
	...
	CPU_GPIO_EnableInputPin(AT91_SPI1_NSS, FALSE, NULL, GPIO_INT_NONE, RESISTOR_PULLUP);
#endif
    return TRUE;
}
...
BOOL AT91_SPI_Driver::Xaction_Start(const SPI_CONFIGURATION &Configuration)
{
    ...
    if(!g_AT91_SPI_Driver.m_Enabled[Configuration.SPI_mod])
    {
        ...
        spi.SPI_MR = AT91_SPI::SPI_MR_MSTR | AT91_SPI::SPI_MR_CS1;
        spi.SPI_CSR1 = AT91_SPI::ConfigurationToCSR(Configuration);
		
#else
		// first build the mode register
        spi.SPI_MR = AT91_SPI::SPI_MR_MSTR | AT91_SPI::SPI_MR_CS0 | (1<<4) | (1<<24);
...
//		if((Configuration.DeviceCS != GPIO_PIN_NONE) && (Configuration.DeviceCS != AT91_SPI0_NSS))
		if((Configuration.DeviceCS != GPIO_PIN_NONE))	// Use GPIO for the function of CS pin to keep it always active during the whole read/write access operation
#endif		
        {
