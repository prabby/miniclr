...
namespace Microsoft.SPOT
{
    ...
    public sealed class Dispatcher
    {
        ...
        public static Dispatcher FromThread(Thread thread)
        {
            ...
            // In addition, in the TinyCLR, all internal calls, such as WeakReference.get_Target
            // are executed in entirety before returning. As such, it is safe to set and retrieve
            // the last accessed dispatcher from outside the global lock.
            ...
            if (dispatcher == null || dispatcher._thread != thread)
            {
                lock (typeof(GlobalLock))
                {
                    ...
                    ArrayList dispatchers = _dispatchers;

                    for (int i = dispatchers.Count; i-- > 0; )
                    {
                        Dispatcher d = ((WeakReference)dispatchers[i]).Target as Dispatcher;
                        if (d != null)
                        {
                            if (d._thread == thread)
                            {
                                ...
                                //
                                // indicate we can update the cached dispatcher
                                fUpdateCache = true;
        ...
        private Dispatcher()
        {
            ...
            _possibleDispatcher.Target = this;
        }
        ...
        public static void PushFrame(DispatcherFrame frame)
        {
            ...
            try
            {
                while (frame.Continue)
                {
                    ...
                    lock (dispatcher._instanceLock)
                    {
                        if (dispatcher._queue.Count > 0)
                        {
                            op = (DispatcherOperation)dispatcher._queue.Dequeue();
                    ...
                    if (op != null)
                    {
                        if (!aborted)
                        {
                            ...
                            {
                                if (dispatcher._finalExceptionHandler == null ||
                                        !dispatcher._finalExceptionHandler(op, e))
                                {
                                    throw;
                                }
                            }
            ...
            finally
            {
                ...
                {
                    if (dispatcher._hasShutdownStarted)
                    {
                        dispatcher.ShutdownImpl();
                    }
                }
            }
        }
        ...
        static ArrayList _dispatchers = new ArrayList();
        static WeakReference _possibleDispatcher = new WeakReference(null);
