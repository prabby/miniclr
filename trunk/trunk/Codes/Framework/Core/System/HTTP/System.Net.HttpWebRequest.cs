...
namespace System.Net
{
    ...
    public class HttpWebRequest : WebRequest
    {
            ...
            if (count > 0)
            {
                ...
                lock (m_ConnectedStreams)
                {
                    ...
                    for (int i = count-1; i >= 0; i--)
                    {
                        ...
                        if (streamWrapper.m_InUse) continue;
        ...
        private InputNetworkStreamWrapper EstablishConnection(Uri proxyServer, Uri targetServer)
        {
                ...
                for (int i = 0; i < m_ConnectedStreams.Count; i++)
                {
                    ...
                    if (inputStream.m_rmAddrAndPort == remoteServer && !inputStream.m_InUse)
                    {
                        ...
                        try
                        {
                            ...
                            if (!inputStream.m_Socket.Poll(1, SelectMode.SelectError))
                            {
            ...
            InputNetworkStreamWrapper retStream = new InputNetworkStreamWrapper(new NetworkStream(socket), socket, !isHttps, proxyServer.Host + ":" + proxyServer.Port);
            ...
            if (isHttps)
            {
                ...
                sslStream.AuthenticateAsClient(m_originalUrl.Host, null, m_caCerts, SslVerification.NoVerification, SslProtocols.Default);
