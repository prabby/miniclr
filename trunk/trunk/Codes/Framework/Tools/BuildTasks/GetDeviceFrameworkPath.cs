...
namespace Microsoft.SPOT.Tasks
{
    public sealed class GetDeviceFrameworkPaths : Task
    {
        ...
        private string frameworkAssembliesPath;
        ...
        public override bool Execute()
        {
            try
            {
                ...
                installRoot = GetDeviceFrameworkValue(runtimeVersion, null, RegistryValues.InstallRoot);
                if (installRoot == null)
                {
                ...
                else
                {
                    ...
                    //TODO: extend this to support the AssemblyFoldersEx technique for OEMs
                }

                FileInfo mscorlibInfo = new FileInfo(Path.Combine(frameworkAssembliesPath, @"mscorlib.dll"));
                ...
                if (!Directory.Exists(frameworkAssembliesPath) || !mscorlibInfo.Exists)
                    throw new MfSdkInternalError(String.Format("the directory \"{0}\" that should contain the MF SDK assemblies does not exist or is not fully installed",frameworkAssembliesPath));
