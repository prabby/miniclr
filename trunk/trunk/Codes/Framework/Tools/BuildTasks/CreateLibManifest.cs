...
using _BE = Microsoft.Build.BuildEngine;
...
namespace Microsoft.SPOT.Tasks
{
    public class CreateLibManifest : Task
    {
        ...
        public override bool Execute()
        {
            try
            {
                _BE.Project proj = new _BE.Project();

                proj.Load(m_projectFile);

                using(StreamWriter sw = new StreamWriter( m_manifestFile.ItemSpec ))
                {
                    sw.WriteLine("<Project xmlns=\"http://schemas.microsoft.com/developer/msbuild/2003\" ToolsVersion=\"3.5\">");
                    sw.WriteLine();
                    ...
                    foreach (_BE.BuildPropertyGroup bpg in proj.PropertyGroups)
                    {
                        if (bpg.IsImported) continue;

                        foreach (_BE.BuildProperty bp in bpg)
                        {
                    ...
                    foreach (_BE.Import imp in proj.Imports)
                    {
                        if (imp.IsImported) continue;

                        if (Path.GetExtension(imp.ProjectPath).ToLower().Trim() == ".libcatproj")
                        {
                            sw.WriteLine("<Import Project=\"{0}\"/>", imp.ProjectPath);
                        }
                    }
